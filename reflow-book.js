...
function paginate(fitW, fitH) {
  ...
  for (const node of sourceNodes) {
    ...
    if (currentPageContent.scrollHeight > currentPageContent.clientHeight) {
      // Overflowed — remove from current page, start new page
      ...
      // If single element is taller than a page, keep it (will clip)
      if (currentPageContent.scrollHeight > currentPageContent.clientHeight) {
        pages.push(extractPageNodes(currentPageContent));
        currentPageContent = createMeasurePage(fitH);
      }
...
function buildBook() {
  ...
  backDiv.className = 'page page-cover';
  backDiv.style.background = 'linear-gradient(135deg, #764ba2 0%, #667eea 100%)';

...
function paginate(fitW: number, fitH: number): Node[][] {
...
function createMeasurePage(fitH: number): HTMLDivElement {
...
function extractPageNodes(pageContent: HTMLElement): Node[] {
  const nodes: Node[] = [];
...
function buildBook(): void {
  ...
  pageFlip.on('flip', () => {
    updatePageInfo();
    trackEvent('page_flip', { page: pageFlip!.getCurrentPageIndex() + 1, total: totalPages });
  });
  updatePageInfo();
  trackEvent('book_loaded', { pages: totalPages, font_scale: fontScale });
  ...
  // Demo button click handler (moved from inline onclick for CSP compliance)
  bookEl.querySelectorAll('.demo-btn').forEach((btn) => {
...
function applyFontScale(): void {
  ...
  buildBook();
  trackEvent('font_size_change', { scale: fontScale, percent: Math.round(fontScale * 100) });
}
